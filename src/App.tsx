...
function App() {
  ...
  const toggleTheme = () => {
    const newDarkMode = !isDarkMode; //since state gets updated later and need new state in localStorage.setItem
    setIsDarkMode(newDarkMode);
    localStorage.setItem("theme", newDarkMode ? "dark" : "light");
    setShowModal(false);
  };

  function showShouldThemeChange() {
    setOpenSlide(false); //close slide
    setShowModal(true); //open confirm modal
  }

  return (
    <>
      <Modal open={showModal} classNames="modal-setting">
        <div>
          <p>Are you sure you want to change theme ?</p>
          <div
            style={{
              display: "flex",
          ...
          >
            <button onClick={toggleTheme}>Yes</button>
            <button
      ...
      <SlideIn
        ...
        contentStyle={{ minHeight: 100, width: 200 }}
      >
        <MenuSlide setOpen={setOpenSlide} handleFunc={showShouldThemeChange} />
      </SlideIn>
      <nav className="nav">
        <ul>
          <li>
