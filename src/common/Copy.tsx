...
const CopyButton = ({
  ...
  function handleCopyText() {
    // console.log(copyBtnRef.current?.textContent?.trim());
    ...
    if (element && element?.textContent) {
      data = element?.textContent.replace(/\s+/g, " ").trim();
    }
    ...
    setTimeout(() => {
      changeText("copy", false);
    }, 1000);
  }

  function changeText(str: string, disable: boolean) {
    if (copyBtnRef.current) {
      const svgElement = copyBtnRef.current.querySelector("svg");
      //   console.log("svgElement", svgElement);

      // Create a new div element to hold both the SVG and the text
      const newContent = document.createElement("div");
      newContent.classList.add("copy-btn__inner");
      // Append the SVG to the new div
      if (svgElement) {
        newContent.appendChild(svgElement.cloneNode(true));
      }

      // Append the text "copied" to the new div
      newContent.appendChild(document.createTextNode(str));

      // Replace the contents of the button with the new div
      copyBtnRef.current.innerHTML = "";
      copyBtnRef.current.appendChild(newContent);

      // Disable the button
      copyBtnRef.current.disabled = disable;
    }
  }
